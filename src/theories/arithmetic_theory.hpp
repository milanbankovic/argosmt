...
#ifndef _ARITHMETIC_THEORY_H
#define _ARITHMETIC_THEORY_H
...
{
  DECLARE_STATIC(mpz_class, temp);
  DECLARE_STATIC(mpq_class, result);
  temp = value;
  result = temp;
...
inline unknown_value && trunc_value(const unknown_value & value)
{
  DECLARE_STATIC(hybrid_integer<int>, temp);
  DECLARE_STATIC(hybrid_rational<int>, result);
...
inline unknown_value && trunc_value(const unknown_value & value)
{
  DECLARE_STATIC(hybrid_integer<long>, temp);
  DECLARE_STATIC(hybrid_rational<long>, result);
...
{
  DECLARE_STATIC(unknown_value, retval);
...
class arithmetic_theory : public theory, private arithmetic_canonizer<unknown_value> {
private:
  enum bound_type {
  ...
  class arithmetic_theory_data : public expression_data {
  private:
  ...
  public:
    arithmetic_theory_data()
      :_constraint_data(0),
       _unknown_data(0)
    {}

    arithmetic_theory_data(const unsigned & level, unsigned index, bool is_integer = false)
      :_constraint_data(0),
       _unknown_data(new unknown_data(level, index, is_integer))
    {}

    arithmetic_theory_data(const expression & u, value_pair && b, bound_type t)
    ...
    void sort_constraints(arithmetic_theory * theory)
    {
      ...
      for(; index < csts.size(); index++)
	{
	  ...
	  const constraint & c_cst = theory->get_theory_data(c)->get_constraint();
	  int i;
	  for(i = (int)index - 1; i >= 0 && 
		c_cst < theory->get_theory_data(csts[i])->get_constraint(); i--)
	    csts[i + 1] = csts[i];
	  csts[i + 1] = c;
	}
    }
  ...
  public:
    unknown_comparator(arithmetic_theory * theory)
      :_theory(theory)
    {}

    bool operator () (const expression & e1, const expression & e2)
    {
      return _theory->get_theory_data(e1)->get_index() < _theory->get_theory_data(e2)->get_index();
    }
  };
  ...
  private:
    arithmetic_theory * _theory;
  public:
    first_basic_unknown_selector(arithmetic_theory * th)
      :_theory(th)
    {}
...
#ifdef _PARALLEL_SIMPLEX
    struct closest_bu_body {
      bool found;
      value_pair min_distance;
      unsigned min_length;
      expression bu;
      unsigned bu_index;
      closest_basic_unknown_selector * cbu_selector;
      closest_bu_body(closest_basic_unknown_selector * cbus)
	:found(false),
	 min_length(0),
	 bu_index((unsigned)(-1)),
	 cbu_selector(cbus)
      {}
      closest_bu_body(closest_bu_body & body, tbb::split)
	:found(false),
	 min_length(0),
	 bu_index((unsigned)(-1)),
	 cbu_selector(body.cbu_selector)
      {}
      void operator () (const tbb::blocked_range<unsigned> & r);
      void join(closest_bu_body & body);    
    };
#endif

  public:
    closest_basic_unknown_selector(arithmetic_theory * th, unsigned lmt = 0)
      :_theory(th),
       _counter(0),
       _limit(lmt)
    {}
    ...
    {
      _counter = 0;
      _limit = _theory->_non_basic_unknowns.size();
    }
  ...
  private:
    arithmetic_theory * _theory;
  public:
    first_non_basic_unknown_selector(arithmetic_theory * th)
      :_theory(th)
    {}
  ...
  private:
    ...
    unsigned long long _limit;    
  public:
    least_frequent_non_basic_unknown_selector(arithmetic_theory * th, unsigned lmt = 0)
      :_theory(th),
       _counter(0),
       _limit(lmt)       
    {}
  ...
  data_extractor<arithmetic_theory_data> _theory_data;
  unknown_set _basic_unknowns;
#ifdef _PARALLEL_SIMPLEX
  hash_vector<expression> _basic_unknowns_vector;
#endif
  ...
  backjump_stack<expression> _assertions;
  ...
  {
    const value_pair & val = get_theory_data(unknown)->get_valuation();
  ...
  value_pair get_strongest_lower_bound_on_trail(arithmetic_theory_data * data);
  value_pair get_strongest_upper_bound_on_trail(arithmetic_theory_data * data);
  ...
  void set_theory_data(const expression & e, arithmetic_theory_data * data)
  {
...
#ifdef _PARALLEL_SIMPLEX
  struct bound_refinements_body {
    bounds_map _bmap;
    arithmetic_theory * _theory;
    bound_refinements_body(arithmetic_theory * th)
      :_bmap(HASH_TABLE_SIZE),
       _theory(th)
    {}
    bound_refinements_body(bound_refinements_body & body, tbb::split)
      :_bmap(HASH_TABLE_SIZE),
       _theory(body._theory)
    {}
    void operator () (const tbb::blocked_range<unsigned> & r);
    void join(bound_refinements_body & body);    
  };
#endif
  
public:
  arithmetic_theory(solver & sl, const std::string & buss = "closest",
    ...
    _check_call_count(0)
  {
    ...
    _assertions.reserve(10000);
  }
  
  arithmetic_theory_data * get_theory_data(const expression & e)
  {
  ...
  {
    //if(_solver.get_literal_data(l)->get_source_theory() == this)
    //  return;
    _assertions.push(l);
  }
  ...
  {
    ostr << "-------- ARITHMETIC THEORY ---------" << std::endl;
	...
	arithmetic_theory_data * cdata = get_theory_data(_all_unknowns[i]);
    ...
    for(expression u : _inactive_unknowns)
      {
	std::cout << get_theory_data(u)->get_constraints().size() << " : " << get_theory_data(u)->get_lower_bound() << " , " << get_theory_data(u)->get_upper_bound() << std::endl;
      }
    */
  }
};

#endif // _ARITHMETIC_THEORY_H
