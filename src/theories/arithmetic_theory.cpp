...
void arithmetic_theory::closest_basic_unknown_selector::
closest_bu_body::operator () (const tbb::blocked_range<unsigned> & r)
{
  hash_vector<expression> & vec = cbu_selector->_theory->_basic_unknowns_vector;
  DECLARE_STATIC(value_pair, curr_distance);
  for(unsigned i = r.begin(); i != r.end(); ++i)
    {
      arithmetic_theory_data * curr_data = cbu_selector->_theory->get_theory_data(vec[i]);
...
void arithmetic_theory::closest_basic_unknown_selector::
closest_bu_body::join(closest_bu_body & body)
{  
  if(body.found && (!found || (body.min_distance < min_distance ||
			       (body.min_distance == min_distance &&
				(body.min_length < min_length ||
				 (body.min_length == min_length &&
				  body.bu_index < bu_index))))))
    {
      min_distance = std::move(body.min_distance);
      bu = std::move(body.bu);
      min_length = body.min_length;
      bu_index = body.bu_index;
      found = true;
    }
}
#endif
...
bool arithmetic_theory::closest_basic_unknown_selector::
select_basic_unknown(expression & bu)
{
  if(_counter < _limit)
    {
      _counter++;
#ifndef _PARALLEL_SIMPLEX
      _theory->_parallel_time_spent.start();
      bool found = false;
      DECLARE_STATIC(value_pair, min_distance);
      DECLARE_STATIC(value_pair, curr_distance);
      unsigned min_length = 0;
      for(unknown_set::const_iterator it = _theory->_basic_unknowns.begin(),
	    it_end = _theory->_basic_unknowns.end(); it != it_end; ++it)
	{
	  arithmetic_theory_data * curr_data = _theory->get_theory_data(*it);
	  bool lv = false, uv = false;
	  if((lv = curr_data->get_valuation() < curr_data->get_lower_bound()) ||
	     (uv = curr_data->get_valuation() > curr_data->get_upper_bound()))
	    {
      ...
      _theory->_parallel_time_spent.acumulate();
      return found;
#else
      closest_bu_body cbu_body(this);
      _theory->_parallel_time_spent.start();
      tbb::parallel_reduce(tbb::blocked_range<unsigned>(0, _theory->_basic_unknowns_vector.size()), cbu_body);
      _theory->_parallel_time_spent.acumulate();
      bu = cbu_body.bu;
      return cbu_body.found;
#endif
    }
  else
    {
      return _theory->find_first_basic_unknown(bu);
    }
}

bool arithmetic_theory::least_frequent_non_basic_unknown_selector::
select_non_basic_unknown(const linear_polynomial & lp, bool lb_violated, expression & nbu)
{
  if(_counter < _limit)
	...
	{
	  if(jt->first == _theory->_one_expression)
	    continue;
	  arithmetic_theory_data * curr_data = _theory->get_theory_data(jt->first);	     
	...
	return false;
      ...
      for(unknown_set::const_iterator it = _theory->_basic_unknowns.begin(),
	    it_end = _theory->_basic_unknowns.end(); it != it_end; ++it)
	{
	  const linear_polynomial & bu_lp = _theory->get_theory_data(*it)->row();
...
#else
      _theory->_parallel_time_spent.start();
      tbb::parallel_for(tbb::blocked_range<unsigned>(0, counters.size()), [&counters, this]
			(const tbb::blocked_range<unsigned> & r)
			{
			  for(unknown_set::const_iterator it = _theory->_basic_unknowns.begin(),
				it_end = _theory->_basic_unknowns.end(); it != it_end; ++it)
			    {
			      ...
			      for(unsigned i = r.begin(); i != r.end(); ++i)
				{
				  if(bu_lp.get_coeff(counters[i].first) != 0)
				     counters[i].second++;
				}
			    }
      ...
      for(auto jt = counters.begin(), jt_end = counters.end(); jt != jt_end; ++jt)
	{
	  if(jt->second < min || (jt->second == min && _theory->get_theory_data(jt->first)->get_index() < min_index))
	    {
	      min = jt->second;
	      nbu = jt->first;
	      min_index = _theory->get_theory_data(nbu)->get_index();
	    }
	}
      return true;
    }
    ...
    {
      return _theory->find_first_non_basic_unknown(lp, lb_violated, nbu);
    }
}


void arithmetic_theory::shuffle_unknowns()
{
  ...
  for(unsigned i = 0; i < _all_unknowns.size(); i++)
    {
      arithmetic_theory_data * u_data = get_theory_data(_all_unknowns[i]);
      u_data->set_index(i);
      if(u_data->is_basic())
	{
...
value_pair arithmetic_theory::calculate_polynomial_value(const linear_polynomial & lp)
{
  value_pair res;
  ...
  for(linear_polynomial::const_iterator it = lp.begin(), it_end = lp.end(); it != it_end; ++it)
	...
	res += get_theory_data(it->first)->get_valuation() * it->second;
    }
  return res;
}

void arithmetic_theory::activate_unknown(const expression & unknown)
{
  if(_inactive_unknowns.find(unknown) != _inactive_unknowns.end())
    {
      arithmetic_theory_data * u_data = get_theory_data(unknown);
...
#ifdef _PARALLEL_SIMPLEX
      _basic_unknowns_vector.push_back(unknown);
#endif
...
void arithmetic_theory::invalidate_inactive_unknowns()
{
  for(auto & p : _inactive_unknowns)
    p.second = false;
}

const value_pair & arithmetic_theory::recalculate_unknown_value(const expression & unknown)
{
  arithmetic_theory_data * u_data = get_theory_data(unknown);
...
void arithmetic_theory::expand_polynomial(linear_polynomial & lp)
{
  ...
  for(linear_polynomial::const_iterator it = lp.begin(), it_end = lp.end(); it != it_end; ++it)
      ...
      if(it->first == _one_expression)
	{
      ...
      arithmetic_theory_data * curr_data = get_theory_data(it->first);
...
void arithmetic_theory::init_monomial_unknowns(const linear_polynomial & lp)
{
  for(linear_polynomial::const_iterator it = lp.begin(), it_end = lp.end(); it != it_end; ++it)
    {
      if(it->first == _one_expression)
	continue;
      ...
      if(!has_theory_data(it->first))
	init_unknown(it->first);
    }
}

void arithmetic_theory::init_unknown(const expression & unknown)
{
  if(has_theory_data(unknown))
    return;
  ...
  arithmetic_theory_data * u_data = new arithmetic_theory_data(_current_level, _all_unknowns.size(), is_integer);
  linear_polynomial & lp = u_data->row();
  set_theory_data(unknown, u_data);
...
void arithmetic_theory::search_unknowns_rec(const expression & e)
{
...
void arithmetic_theory::add_literal(const expression & l, const expression & l_opp)
{
  if(has_theory_data(l))
    return;

  if(is_arithmetic_relation(l) || 
     (l->is_equality() && is_lra_term(l->get_operands()[0]) && is_lra_term(l->get_operands()[1])))
    {
      _solver.get_literal_data(l)->add_observing_theory(this);
    }
  ...
  if(unknown == _zero_expression)
    {
      ...
      set_theory_data(l, new arithmetic_theory_data());
      set_theory_data(l_opp, new arithmetic_theory_data());
      return;
    }
  ...
  arithmetic_theory_data * l_data = new arithmetic_theory_data(unknown, std::move(rp), bt);
  set_theory_data(l, l_data);
  ...
  arithmetic_theory_data * l_opp_data = new arithmetic_theory_data(unknown, std::move(rp), bt);
  set_theory_data(l_opp, l_opp_data);
  u_data->add_constraint(l_opp);       
  activate_unknown(unknown);
}

void arithmetic_theory::new_level()
{
  _new_level_time_spent.start();
  _check_count++;
  _current_level++;
  assert(_current_assertion_pos == _assertions.size());
  _assertions.new_level();
  _new_level_time_spent.acumulate();
}

void arithmetic_theory::backjump(unsigned level)
{
  _backjump_time_spent.start();
  _current_level = level;
  for(unsigned i = 0; i < _all_unknowns.size(); i++)
    {
      get_theory_data(_all_unknowns[i])->restore_state();
    }
...
void arithmetic_theory::explain_literal(const expression & l)
{
  _explain_time_spent.start();
  arithmetic_theory_data * l_data = get_theory_data(l);
...
void arithmetic_theory::process_assertions()
{
...
void arithmetic_theory::process_assertion(const expression & l)
{
  assert(l->is_equality() || l->is_disequality() || is_arithmetic_relation(l));
  assert(has_theory_data(l));
  ...
  assert(!l_data->has_constraint_data() || !get_theory_data(l_data->get_unknown())->is_integer() ||
	 (
	  (get_theory_data(l_data->get_unknown())->get_lower_bound().infinity != I_NONE ||
	   (is_integer_value(get_theory_data(l_data->get_unknown())->get_lower_bound().first) &&
	    get_theory_data(l_data->get_unknown())->get_lower_bound().second == 0))
	  &&
	  (get_theory_data(l_data->get_unknown())->get_upper_bound().infinity != I_NONE ||
	   (is_integer_value(get_theory_data(l_data->get_unknown())->get_upper_bound().first) &&
	    get_theory_data(l_data->get_unknown())->get_upper_bound().second == 0))
	  )
	 );
}

void arithmetic_theory::assert_upper(const expression & l)
{
  ...
  if(is_fixed_by_constraints(unknown))
    {
      DECLARE_STATIC(constraint, c_both);
      c_both.bound = bound;
      c_both.type = B_BOTH;
...
void arithmetic_theory::assert_lower(const expression & l)
{
...
void arithmetic_theory::assert_both(const expression & l)
{
...
void arithmetic_theory::assert_split(const expression & l)
{
  if(_solver.get_trail().get_source_theory(l) != this)
    {      
      _disequality_asserted = true;
      arithmetic_theory_data * l_data = get_theory_data(l);
      const expression & unknown = l_data->get_unknown();
...
void arithmetic_theory::split_on_demand(const expression & l)
{
  arithmetic_theory_data * l_data = get_theory_data(l);
...
void arithmetic_theory::tighten_upper_integer_bound(value_pair & bound)
{
  DECLARE_STATIC(unknown_value, temp);
...
void arithmetic_theory::tighten_lower_integer_bound(value_pair & bound)
{
  DECLARE_STATIC(unknown_value, temp);
...
void arithmetic_theory::tighten_integer_bound(value_pair & bound, bound_type bt)
{
...
bool arithmetic_theory::check_integer_unknowns()
{
  ...
  for(unsigned i = 0; i < _integer_unknowns.size(); i++)
    {
      ...
      DECLARE_STATIC(value_pair, temp);
      ...
      if(vp.second != 0 || !is_integer_value(vp.first))
	{
	  ...
	  //std::cout << get_theory_data(unknown)->is_basic() << std::endl;
	  temp = vp;
...
void arithmetic_theory::check_and_propagate(unsigned layer)
{
  ...
  assert(_solver.is_conflict() || _current_assertion_pos == _assertions.size());
  _check_and_prop_time_spent.acumulate();
}

expression arithmetic_theory::canonize_expression(const expression & e)
{
  return arithmetic_canonizer<unknown_value>::canonize_expression(e);
}

void arithmetic_theory::get_literal_pair(const expression & l, expression & l_pos, expression & l_neg)
{
  arithmetic_canonizer<unknown_value>::get_literal_pair(l, l_pos, l_neg);
}

void arithmetic_theory::get_model(const expression_vector & exps)
{
  calculate_delta(); 
  for(unsigned i = 0; i < exps.size(); i++)
    {
      if(!has_theory_data(exps[i]) || !get_theory_data(exps[i])->has_unknown_data())
	continue;
...
void arithmetic_theory::found_trivial_propagation(const unknown_value & c, const function_symbol & fs, 
...
void arithmetic_theory::apply_trivial_propagation(const expression & l)
{
...
void arithmetic_theory::pivot(const expression & bu, const expression & nbu)
{
  linear_polynomial & bu_lp = get_theory_data(bu)->row();
  linear_polynomial & nbu_lp = get_theory_data(nbu)->row();
  DECLARE_STATIC(unknown_value, aij);
...
#ifdef _PARALLEL_SIMPLEX
  _basic_unknowns_vector.erase(bu);
#endif
...
#else
  tbb::parallel_for(tbb::blocked_range<unsigned>(0, _basic_unknowns_vector.size()),
		    [this, &nbu, &nbu_lp] (const tbb::blocked_range<unsigned> & r) 
		    {
		      hash_vector<expression> & vec = _basic_unknowns_vector;
		      DECLARE_STATIC(unknown_value, old_akj);
		      for(unsigned i = r.begin(); i != r.end(); ++i)
			{
			  linear_polynomial & c_lp = get_theory_data(vec[i])->row();
			  const unknown_value & akj = c_lp.get_coeff(nbu);
			  if(akj != 0)
			    {
			      old_akj = akj; // remember old akj
  ...
  get_theory_data(nbu)->set_basic(true);
  if(!get_theory_data(nbu)->get_constraints().empty())
    {
      _basic_unknowns.insert(nbu);
#ifdef _PARALLEL_SIMPLEX
      _basic_unknowns_vector.push_back(nbu);
#endif
    }
  else
    _inactive_unknowns.insert(std::make_pair(nbu, false));
  get_theory_data(bu)->set_basic(false);
  _non_basic_unknowns.insert(bu);
}

void arithmetic_theory::update(const expression & nbu, const value_pair & v)
{
  arithmetic_theory_data * nbu_data = get_theory_data(nbu);
  const value_pair & nbu_val = nbu_data->get_valuation();


  _parallel_time_spent.start();  
#ifndef _PARALLEL_SIMPLEX
  DECLARE_STATIC(value_pair, temp); // for calculating aji * (v - nbu_val)
  for(unknown_set::const_iterator it = _basic_unknowns.begin(), it_end = _basic_unknowns.end(); it != it_end; ++it)
    {
      arithmetic_theory_data * bu_data = get_theory_data(*it);
...
#else
  tbb::parallel_for(tbb::blocked_range<unsigned>(0, _basic_unknowns_vector.size()),
		    [this, &nbu, &v, &nbu_val] (const tbb::blocked_range<unsigned> & r)
		    {
		      hash_vector<expression> & vec = _basic_unknowns_vector;		      
		      DECLARE_STATIC(value_pair, temp);  // for calculating aji * (v - nbu_val)		      
		      for(unsigned i = r.begin(); i != r.end(); ++i)
			{
			  arithmetic_theory_data * bu_data = get_theory_data(vec[i]);
			  const unknown_value & aji = bu_data->row().get_coeff(nbu);
			  if(aji == 0) continue;
			  temp = v;
			  temp -= nbu_val;
...
void arithmetic_theory::pivot_and_update(const expression & bu, const expression & nbu, const value_pair & v)
{
  _pivot_and_update_time_spent.start();
  
  arithmetic_theory_data * bu_data = get_theory_data(bu);
  const value_pair & bu_val = bu_data->get_valuation();
  const unknown_value & aij = bu_data->row().get_coeff(nbu);
  arithmetic_theory_data * nbu_data = get_theory_data(nbu);
  
  
  DECLARE_STATIC(value_pair, theta);   // theta = (v - bu_val) / aij;
  theta = v;
  theta -= bu_val;
  theta /= aij;
  ...
  _parallel_time_spent.start();
#ifndef _PARALLEL_SIMPLEX
  DECLARE_STATIC(value_pair, temp);  // temp = akj * theta;
  for(unknown_set::const_iterator it = _basic_unknowns.begin(), it_end = _basic_unknowns.end(); it != it_end; ++it)
    {
      if(*it == bu) continue;      
      arithmetic_theory_data * c_data = get_theory_data(*it);      
...
#else  
  tbb::parallel_for(tbb::blocked_range<unsigned>(0, _basic_unknowns_vector.size()),
		    [this, &bu, &nbu, &theta](const tbb::blocked_range<unsigned> & r)
		    {
		      hash_vector<expression> & vec = _basic_unknowns_vector;
		      DECLARE_STATIC(value_pair, temp);  // temp = akj * theta;
		      for(unsigned i = r.begin(); i != r.end(); ++i)
			{
			  if(vec[i] == bu) continue;			  
			  arithmetic_theory_data * c_data = get_theory_data(vec[i]);
			  const unknown_value & akj = c_data->row().get_coeff(nbu);
			  if(akj == 0) continue; 
			  temp = theta;
			  temp *= akj;
...
bool arithmetic_theory::find_first_non_basic_unknown(const linear_polynomial & lp, bool lb_violated, expression & first)
{
...
bool arithmetic_theory::find_first_basic_unknown(expression & first)
{
  for(unknown_set::const_iterator it = _basic_unknowns.begin(), it_end = _basic_unknowns.end(); it != it_end; ++it)
    {
      arithmetic_theory_data * curr_data = get_theory_data(*it);
...
bool arithmetic_theory::check(expression & bu)
{
  if(!_bound_changed)
    return true;
...
struct nbu_lower_comp {
  arithmetic_theory * _theory;
  nbu_lower_comp(arithmetic_theory * th) :_theory(th) {}
		    ...
		    const value_pair & bound2, const expression & bu2) const
  {
    return bound1 > bound2 || (bound1 == bound2 &&
			       ...
			       _theory->get_theory_data(bu1)->get_index() <
			       _theory->get_theory_data(bu2)->get_index());     
  }
};

struct nbu_upper_comp {
  arithmetic_theory * _theory;
  nbu_upper_comp(arithmetic_theory * th) :_theory(th) {}
  bool operator () (const value_pair & bound1, const expression & bu1,
		    const value_pair & bound2, const expression & bu2) const
  {
    return bound1 < bound2 || (bound1 == bound2 &&
...
void arithmetic_theory::calculate_bounds(const expression & bu, bounds_map & bmap)
{
  assert(has_theory_data(bu));
  assert(get_theory_data(bu)->has_unknown_data());
  assert(get_theory_data(bu)->is_basic());

  arithmetic_theory_data * bu_data = get_theory_data(bu);
  linear_polynomial & bu_lp = bu_data->row();
  DECLARE_STATIC(value_pair, lower);
  DECLARE_STATIC(value_pair, upper);
  expression upp_hole;
  expression low_hole;
  ...
  DECLARE_STATIC(value_pair, nbu_lower);
  DECLARE_STATIC(value_pair, nbu_upper);
  DECLARE_STATIC(value_pair, temp);      
...
#ifdef _PARALLEL_SIMPLEX
void arithmetic_theory::bound_refinements_body::operator () (const tbb::blocked_range<unsigned> & r)
{
  hash_vector<expression> & vec = _theory->_basic_unknowns_vector;
  for(unsigned i = r.begin(); i != r.end(); ++i)
    {
      const expression & bu = vec[i];
      _theory->calculate_bounds(bu, _bmap);
    }  
}

void arithmetic_theory::bound_refinements_body::join(bound_refinements_body & body)
{
  for(const auto & p : body._bmap)
    {
      auto it = _bmap.find(p.first);
      if(it != _bmap.end())
	{
	  if(nbu_lower_comp(_theory)(p.second.lower, p.second.lower_bu,
				     it->second.lower, it->second.lower_bu))
	    {
	      it->second.lower = std::move(p.second.lower);
	      it->second.lower_bu = std::move(p.second.lower_bu);
	    }
	  if(nbu_upper_comp(_theory)(p.second.upper, p.second.upper_bu,
				     it->second.upper, it->second.upper_bu))
	    {
	      it->second.upper = std::move(p.second.upper);
	      it->second.upper_bu = std::move(p.second.upper_bu);
	    }
	}
      else
	_bmap[p.first] = std::move(p.second);
    }
}
#endif

void arithmetic_theory::propagate_bound_refinements()
{
  if(!_bound_changed)
    return;
  ...
  DECLARE_STATIC(constraint, lo_cst);
  DECLARE_STATIC(constraint, up_cst);
#ifndef _PARALLEL_SIMPLEX
  DECLARE_STATIC(bounds_map, bmap);
  bmap.clear();
...
#else
  bound_refinements_body br_body(this);
  _parallel_time_spent.start();
  tbb::parallel_reduce(tbb::blocked_range<unsigned>(0, _basic_unknowns_vector.size()), br_body);
  _parallel_time_spent.acumulate();

  bounds_map & bmap = br_body._bmap;
  for(unknown_set::const_iterator it = _basic_unknowns.begin(), it_end = _basic_unknowns.end(); it != it_end; ++it)
    {
      const expression & bu = *it;
      arithmetic_theory_data * bu_data = get_theory_data(bu);
...
void arithmetic_theory::propagate_weaker_constraints(const expression & unknown, const constraint & c)
{
  _weaker_time_spent.start();  
  get_theory_data(unknown)->sort_constraints(this);
  const expression_vector & csts = get_theory_data(unknown)->get_constraints();
  ...
  for(int i = low; i <= up; i++)
	...
	continue;
      ...
      arithmetic_theory_data * curr_data = get_theory_data(csts[i]);
	  ...
	  if(cstsi_value == EB_UNDEFINED)	  
	    {
	      ...
	      get_theory_data(csts[i])->literal_explanation() = expl;
	    }
...
void arithmetic_theory::introduce_equalities()
{
  ...
  std::sort(_eq_relevant_unknowns.begin(), _eq_relevant_unknowns.end(),
	    ...
	    {
	      arithmetic_theory_data * e1_data = get_theory_data(e1);
	      arithmetic_theory_data * e2_data = get_theory_data(e2);	      
  ...
  for(unsigned i = 0; i < _eq_relevant_unknowns.size() - 1; i++)
	...
	{
	  if(get_theory_data(_eq_relevant_unknowns[i])->get_valuation() == get_theory_data(_eq_relevant_unknowns[j])->get_valuation())
	    {
	      expression eq;	      
...
int arithmetic_theory::find_constraint_r(const expression_vector & csts, const constraint & cst)
{
  ...
  while(l < r)
    {
      s = (l + r) >> 1;
      const constraint & s_cst = get_theory_data(csts[s])->get_constraint();
      if(cst < s_cst)
	r = s;
      else 
	l = s + 1;
...
int arithmetic_theory::find_constraint_l(const expression_vector & csts, const constraint & cst)
{
  int l = 0, r = (int)csts.size();
  int s;
  while(l < r)
...
void arithmetic_theory::check_equalities()
{
  ...
  for(unsigned i = 0; i < _all_unknowns.size(); i++)
      ...
      if(!is_fixed_by_constraints(_all_unknowns[i]) && !is_poly_fixed_by_constraints(_all_unknowns[i]))
	{
	  ...
	  while(j >= 0 && get_theory_data(u_csts[j])->get_constraint() == cst)
	    {
...
void arithmetic_theory::generate_bound_explanation(const expression & unknown, bound_type bt, explanation & expl)
{
  arithmetic_theory_data * u_data = get_theory_data(unknown);
  DECLARE_STATIC(duplicate_elimination_wrapper, wrapper);
  wrapper.reset(expl);
  assert(bt != B_SPLIT);
  ...
  if(u_data->is_basic())
      ...
      if(bt == B_UPPER || bt == B_BOTH)
	{
	  ...
	  arithmetic_theory_data * upper_bu_data = get_theory_data(upper_bu);
      ...
      if(bt == B_LOWER || bt == B_BOTH)
	{
	  ...
	  arithmetic_theory_data * lower_bu_data = get_theory_data(lower_bu);
...
bool arithmetic_theory::generate_simple_explanation(const expression & e, explanation & expl)
{
  arithmetic_theory_data * e_data = get_theory_data(e);
  ...
  if(u_data->get_lower_bound().infinity == I_NONE)
    {
      ...
      const constraint & c_lower = get_theory_data(lower_rel)->get_constraint();
  ...
  if(u_data->get_upper_bound().infinity == I_NONE)
    {
      ...
      const constraint & c_upper = get_theory_data(upper_rel)->get_constraint();
  ...
  if(c.type == B_SPLIT)
    {
      ...
      while(j >= 0 && get_theory_data(u_csts[j])->get_constraint() == c)
	{
	  if(_solver.get_trail().get_value(u_csts[j]) == EB_TRUE)
	    {
...
void arithmetic_theory::generate_propagation_explanation(const expression & e, const constraint & sc, explanation & expl)
{
  assert(has_theory_data(e));
  assert(_solver.has_literal_data(e));
  assert(get_theory_data(e)->has_constraint_data());
...
void arithmetic_theory::generate_conflict_explanation(const expression & bu, explanation & expl)
{
  assert(has_theory_data(bu));
  assert(get_theory_data(bu)->has_unknown_data());
  assert(get_theory_data(bu)->is_basic());
...
void arithmetic_theory::calculate_delta()
{
...
value_pair arithmetic_theory::get_strongest_lower_bound_on_trail(arithmetic_theory_data * data)
{
  ...
  for(unsigned i = 0; i < csts.size(); i++)
	...
	{
	  arithmetic_theory_data * c_data = get_theory_data(csts[i]);
...
value_pair arithmetic_theory::get_strongest_upper_bound_on_trail(arithmetic_theory_data * data)
{
  const expression_vector & csts = data->get_constraints();
...
void arithmetic_theory::expand_to_lra_unknowns(linear_polynomial & lp)
{
...
void arithmetic_theory::get_tableau_statistics()
{
  DECLARE_STATIC(unknown_integer_value, abs_value);
...
void arithmetic_theory::tableau_report(std::ostream & ostr)
{
...
void arithmetic_theory::print_tableau()
{
  for(unknown_set::const_iterator jt = _basic_unknowns.begin(), jt_end = _basic_unknowns.end();
      jt != jt_end; ++jt)
    {
      arithmetic_theory_data * th_data = get_theory_data(*jt);
	...
	{
	  std::cout << "[" << it->second << " * x_" << get_theory_data(it->first)->get_index() << "] ";
	}
      std::cout << " ]" << std::endl;
      */
  ...
  for(unsigned i = 0; i < _all_unknowns.size(); i++)
    {
      arithmetic_theory_data * th_data = get_theory_data(_all_unknowns[i]);
...
bool arithmetic_theory::check_invariant_nb1()
{
...
bool arithmetic_theory::check_invariant_nb2()
{
  unknown_set::const_iterator it = _non_basic_unknowns.begin(), it_end = _non_basic_unknowns.end();
...
bool arithmetic_theory::check_invariant_nb3()
{
  unknown_set::const_iterator it = _non_basic_unknowns.begin(), it_end = _non_basic_unknowns.end();
...
bool arithmetic_theory::check_invariant_nb4()
{
  for(unknown_set::const_iterator it = _non_basic_unknowns.begin(), it_end = _non_basic_unknowns.end(); it != it_end; ++it)
    {
      if(get_theory_data(*it)->is_basic())
	{
	  std::cout << "OUPS: nb4-basic: " << *it << std::endl;
	  return false;
	}
	
      const linear_polynomial & lp = get_theory_data(*it)->row();
...
bool arithmetic_theory::check_invariant_nb5()
{
  for(unknown_set::const_iterator it = _non_basic_unknowns.begin(), it_end = _non_basic_unknowns.end(); it != it_end; ++it)
    {
      arithmetic_theory_data * curr_data = get_theory_data(*it);
...
bool arithmetic_theory::check_invariant_b1()
{
...
bool arithmetic_theory::check_invariant_b2()
{
  unknown_set::const_iterator it = _basic_unknowns.begin(), it_end = _basic_unknowns.end();
...
bool arithmetic_theory::check_invariant_b3()
{
  unknown_set::const_iterator it = _basic_unknowns.begin(), it_end = _basic_unknowns.end();
...
bool arithmetic_theory::check_invariant_b4()
{
  for(unknown_set::const_iterator it = _basic_unknowns.begin(), it_end = _basic_unknowns.end(); it != it_end; ++it)
    {
      if(!get_theory_data(*it)->is_basic())
      ...
      for(linear_polynomial::const_iterator jt = lp.begin(), jt_end = lp.end(); jt != jt_end; jt++)
	{
	  ...
	  if(get_theory_data(jt->first)->is_basic())
	    {
...
bool arithmetic_theory::check_invariant_b5()
{
  for(unknown_set::const_iterator it = _basic_unknowns.begin(), it_end = _basic_unknowns.end(); it != it_end; ++it)
    {
      arithmetic_theory_data * curr_data = get_theory_data(*it);
...
bool arithmetic_theory::check_all_invariants()
{
  bool ret = true;
