...
private:
  ...
  {
    csp_theory::csp_theory_data * v_data = _theory->get_theory_data(v);
    ...
    for(expression_set::const_iterator i = csts.begin(), i_end = csts.end(); i != i_end; ++i)
      {
	if(!_theory->get_theory_data(*i)->get_constraint_handler()->is_active())
	  continue;
	...
	if(it != _csts_at_least_two.end())
	  ...
	  {
	    csp_theory::csp_theory_data * c_data = _theory->get_theory_data(*i);
	    ...
	    for(expression_set::const_iterator j = vars.begin(), j_end = vars.end(); j != j_end; ++j)
	      if(*j != v && _theory->get_theory_data(*j)->get_variable_domain_handler()->current_domain().size() > 1)
		{
		  val = true;
		  csts_count++;
		  break;
...
public:
  most_constrained_csp_variable_order_strategy(csp_theory * th)
  ...
  {
    const expression_vector & vars = _theory->get_variables();
    var = expression();
    unsigned dom_size = 0;
