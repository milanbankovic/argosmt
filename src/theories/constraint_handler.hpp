  ...
  void propagate_weaker_from_pos_constraints()
  {
	...
	if(l_value == EB_UNDEFINED)
	  ...
	  {	    
	    ...
	    const expression & l_opp = _theory->get_solver().get_literal_data(l)->get_opposite();
...
public:
  constraint_handler(csp_theory * th, const expression & c, const expression & nc, bool is_active = false)
