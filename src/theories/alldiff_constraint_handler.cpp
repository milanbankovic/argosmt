...
alldiff_constraint_handler::alldiff_constraint_handler(csp_theory * th, const expression & l_pos, const expression & l_neg, unsigned check_period)
   ...
   _common_data(&th->_alldiff_common_data)
{
  ...
  for(unsigned i = 0; i < ops.size(); i++)
    {
      csp_theory::csp_theory_data * var_data = _theory->add_variable(ops[i]);
...
bool alldiff_constraint_handler::convert_atom_to_edge(const expression & atom, edge & e) const
{
  ...
  int val = _theory->get_theory_data(atom->get_operands()[1])->get_int_value();
...
void alldiff_constraint_handler::convert_edge_to_equality(const edge & e, expression & atom) const
{
  ...
  csp_theory::csp_theory_data * data = _theory->get_theory_data(_variables[left]);
...
void alldiff_constraint_handler::convert_vertex_to_term(const vertex & v, expression & term) const
{
  switch(v.vertex_side())
    ...
    case bipartite_graph::RIGHT:
      ...
      _theory->get_expr_for_int(_values[v.vertex_number()], term);
      break;
    default:
      break;
    }
...
{
  edge e;  
  DECLARE_STATIC(std::vector<edge>, reason);
  reason.clear();
  bipartite_graph & start_graph = get_start_graph();
  const bipartite_graph & graph = get_const_current_graph();
  DECLARE_STATIC(bipartite_graph::edge_marking, trail_marking);
  trail_marking.reset_marking(&start_graph);
  DECLARE_STATIC(bipartite_graph::edge_marking, reason_marking);
  reason_marking.reset_marking(&start_graph);
  DECLARE_STATIC(bipartite_graph::edge_attribute<expression>, edge_cause);
  edge_cause.reset_attributes(&start_graph);
  //multivar_explanation_minimizer & wrapper = _expl_wrapper;
  //wrapper.reset(_theory);
  multivar_explanation_minimizer wrapper(_theory);
...
{    
  ...
  for(unsigned i = 0; i < reason.size(); i++)
	...
	{	    
	  ...
	  domain_handler * d_handler = _theory->get_theory_data(var)->get_variable_domain_handler();
	  ...
	  if(equality.get() != 0 && _theory->get_solver().get_trail().get_value(equality) == EB_TRUE)
	    {
	  ...
	  else
	    {
	      ...
	      assert(_theory->get_solver().get_trail().get_value(disequality) == EB_TRUE);
...
void alldiff_constraint_handler::generate_finite_alldiff_explanation(explanation & expl)
{
  ...
  if(_theory->get_solver().get_trail().get_trail_level(_constraint) > 0)
    expl.push_back(_constraint);

  for(unsigned i = 0; i < _variables.size(); i++)
    {
      csp_theory::csp_theory_data * v_data = _theory->get_theory_data(_variables[i]);
...
void alldiff_constraint_handler::init_handler()
{
  ...
  for(unsigned i = 0; i < vars.size(); i++)
    {            
      csp_theory::csp_theory_data * vi_data = _theory->get_theory_data(vars[i]);
  ...
  _inited_at_level = _theory->_current_level;
  init_start_graph();
  bipartite_graph & start_graph = get_start_graph();

  for(unsigned i = 0; i < _variables.size(); i++)
    {
      domain_handler * vi_handler = _theory->get_theory_data(_variables[i])->get_variable_domain_handler();
      const std::vector<int> & domain = vi_handler->initial_finite_domain();
