...
{
  if(_theory->get_solver().get_trail().get_source_theory(l) != _theory ||
     _theory->get_theory_data(l)->get_explanation_handler() != this)
    {
      _trail.push(l);
    }
}
...
{
  for(unsigned i = 0; i < _next_to_assert && !_theory->get_solver().is_conflict(); i++)
    {
  ...
  while(_min_index <= _max_index && _domain_values.find(_sorted_values[_min_index]) == _domain_values.end())
    {
      ...
      if(!_theory->get_solver().is_conflict() && info._ge.get() != 0)
	propagate_literal(info._ge);
      if(!_theory->get_solver().is_conflict() && info._gt.get() != 0)
	propagate_literal(info._gt);
      
      _min_index++;
    }
  if(_min_index <= _max_index && !_theory->get_solver().is_conflict())
    {
      value_info & info = _all_values[_sorted_values[_min_index]];
      if(info._ge.get() != 0)
	propagate_literal(info._ge);
  ...
  while(_max_index >= _min_index && _domain_values.find(_sorted_values[_max_index]) == _domain_values.end())
    {
      ...
      if(!_theory->get_solver().is_conflict() && info._le.get() != 0)
	propagate_literal(info._le);
      if(!_theory->get_solver().is_conflict() && info._lt.get() != 0)
	propagate_literal(info._lt);

     _max_index--;
    }
  if(_max_index >= _min_index && !_theory->get_solver().is_conflict())
    {
      value_info & info = _all_values[_sorted_values[_max_index]];
      if(info._le.get() != 0)
	propagate_literal(info._le);
...
bool domain_handler::check_bounds(expression & l_cause, expression & u_cause, int & lower, int & upper)
{
  ...
  for(unsigned i = 0; i <= _next_to_assert; i++)
	...
	{
	  int ub = _theory->get_theory_data(li->get_operands()[1])->get_int_value();
	  if(ub < upper)
	    {
	      upper = ub;
	      u_cause = li;
	...
	{
	  int ub = _theory->get_theory_data(li->get_operands()[1])->get_int_value() - 1;
	  if(ub < upper)
	    {
	      upper = ub;
	      u_cause = li;
	...
	{
	  int lb = _theory->get_theory_data(li->get_operands()[1])->get_int_value();
	  if(lb > lower)
	    {
	      lower = lb;
	      l_cause = li;
	...
	{
	  int lb = _theory->get_theory_data(li->get_operands()[1])->get_int_value() + 1;
	  if(lb > lower)
	    {
	      lower = lb;
	      l_cause = li;
...
void domain_handler::check_not_finite(const expression & l)
{
  if(l->is_disequality())
  ...
  else if(l->is_equality())
    {
      ...
      _domain_values.insert(_theory->get_theory_data(l->get_operands()[1])->get_int_value());
      check_all_values();
      init_min_max();
...
void domain_handler::init_literal_pair(int v, bool introduce)
{
  ...
  expression val;
  _theory->get_expr_for_int(v, val);
  expression eq;
  bool is_cached = _theory->get_solver().is_equality_cached(_var, val, eq);
  if(is_cached || introduce)
    {
      if(!is_cached)
	{
	  eq = _theory->get_solver().get_factory()->create_expression(function_symbol::EQ, _var, val);
	  _theory->get_solver().apply_introduce_literal(eq);
	  _common_data->_count_equalities++;
	  //_theory->get_solver().get_literal_selection_strategy()->branching_priority_hint(eq, -5);
	}
      info._eq = eq;
      info._diseq = _theory->get_solver().get_literal_data(eq)->get_opposite();
    }
}
...
void domain_handler::init_upper_bound(int v)
{
  ...
  expression le = _theory->get_solver().get_factory()->create_expression(function_symbol::LE, _var, val);
  if(!_theory->get_solver().has_expression_data(le) || !_theory->get_solver().has_literal_data(le))
    {
      _theory->get_solver().apply_introduce_literal(le);
      _common_data->_count_inequalities++;
      //_theory->get_solver().get_literal_selection_strategy()->branching_priority_hint(le, 5);
    }
  info._le = le;
  info._gt = _theory->get_solver().get_literal_data(le)->get_opposite();
}
...
void domain_handler::init_lower_bound(int v)
{
  ...
  expression ge = _theory->get_solver().get_factory()->create_expression(function_symbol::GE, _var, val);
  if(!_theory->get_solver().has_expression_data(ge) || !_theory->get_solver().has_literal_data(ge))
    {
      _theory->get_solver().apply_introduce_literal(ge);
      _common_data->_count_inequalities++;
      //_theory->get_solver().get_literal_selection_strategy()->branching_priority_hint(ge, 5);
    }
  info._ge = ge;
  info._lt = _theory->get_solver().get_literal_data(ge)->get_opposite();
}
...
{
  DECLARE_STATIC(std::unordered_set<int>, new_domain);
  new_domain.clear();
  new_domain.rehash(HASH_TABLE_SIZE);
  for(std::unordered_set<int>::const_iterator it = _domain_values.begin(),
	it_end = _domain_values.end(); it != it_end && 
	!_theory->get_solver().is_conflict(); ++it)
    {
      if(!f(*it))
	{
	  if(_trail.current_level() != 0)
...
void domain_handler::prune_greater_values(int value, const expression & l)
{
  ...
  if((int)_sorted_values.size() - ind > (int)_domain_values.size())
    ...
    {
      for(; ind <= _max_index && !_theory->get_solver().is_conflict(); ind++)
	{
  ...
  if(l->is_disequality())
    {
      ...
      int value = _theory->get_theory_data(val)->get_int_value();
      prune_value(value, l);
    } 
  else if(l->is_equality())
    {
    ...
    {
      const expression & val = l->get_operands()[1];
      int b = _theory->get_theory_data(val)->get_int_value();
      prune_greater_values(b, l);
    }
  else if(l->get_symbol() == function_symbol::LT)
    {
      const expression & val = l->get_operands()[1];
      int b = _theory->get_theory_data(val)->get_int_value() - 1;
      prune_greater_values(b, l);
    }
...
{
  expl.reserve(10);
  DECLARE_STATIC(explanation_minimizer, wrapper);
  wrapper.reset(_theory);
  //explanation_minimizer wrapper(_theory);
...
{  
  int val = _theory->get_theory_data(l->get_operands()[1])->get_int_value();
...
void domain_handler::explain_minimum_at_trail_size(unsigned size, explanation & expl)
{
  ...
  for(i = get_zero_min_index(); i < _min_index; i++)
    {
      ...
      if(_theory->get_solver().get_trail().get_trail_index(vi._expl) < size)
  ...
  if(i < (int)_sorted_values.size())
    {
      const value_info & vi = _all_values[_sorted_values[i]];
      if(vi._eq.get() != 0 && _theory->get_solver().get_trail().get_value(vi._eq) == EB_TRUE
	 && _theory->get_solver().get_trail().get_trail_index(vi._eq) < size)
	{
