...
#ifndef _CSP_THEORY_H
#define _CSP_THEORY_H
...
class csp_theory : public theory, public arithmetic_canonizer<int> {
public:

  enum csp_expression_type { CSP_CONSTRAINT, CSP_VARIABLE, CSP_VALUE, CSP_DOMAIN_RESTRICTION };

  class csp_theory_data : public expression_data {
  private:
  ...
  public:
    csp_theory_data(csp_expression_type t, unsigned index, 
		    csp_theory_data * data = 0);
...
private:
  ...
  struct on_pop {
    csp_theory * _theory;
    on_pop(csp_theory * th)
      :_theory(th) {}

    void operator () (const expression & e) const;
  };

  data_extractor<csp_theory_data> _data;
...
public:
  csp_theory(solver & sl, alldiff_sum_algorithm as_alg = AS_NONE,
  ...
  csp_theory_data * get_theory_data(const expression & e)
  {
    return _data.get_data(e);
  }
  
  void set_theory_data(const expression & e, csp_theory_data * data)
  {
  ...
  csp_theory_data * add_variable(const expression & var);
  csp_theory_data * add_value(const expression & val);
  csp_theory_data * add_value(int v);
  csp_theory_data * add_constraint_pair(const expression & l_pos, const expression & l_neg);
  csp_theory_data * add_domain_restriction_pair(const expression & l_pos, const expression & l_neg);
