...
private:
  ...
  {
    _start_graph.assign(_variables.size(),_values.size(), &_theory->_bipartite_graph_common_data);
  }
  ...
  {
    return (level > 0 && _check_count >= _check_period) || _theory->get_solver().all_literals_assigned();
  }
  ...
  double calculate_explanation_depth(const explanation & expl) const
  {
    ...
    for(unsigned i = 0; i < expl.size(); i++)
      {
	s += _theory->get_solver().get_trail().current_level() - 
	  _theory->get_solver().get_trail().get_trail_level(expl[i]);
      }
...
public:
  ...
  alldiff_constraint_handler(csp_theory * th, const expression & l_pos, const expression & l_neg, unsigned check_period = 0);
