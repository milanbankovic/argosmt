...
struct explanation_minimizer {
  ...
  explanation_minimizer(csp_theory * t = 0)
    :set(HASH_TABLE_SIZE),
  ...
  void push_back(const expression & e)
  {
	...
	set.insert(e);
      ...
      {
	int b = th->get_theory_data(e->get_operands()[1])->get_int_value();
	if(b <= upper)
	  {
	    upper = b;
	    u_cause = e;
	...
	if(b >= lower)
      ...
      {
	int ub = th->get_theory_data(e->get_operands()[1])->get_int_value();
	if(ub < upper)
	  {
	    upper = ub;
	    u_cause = e;
      ...
      {
	int ub = th->get_theory_data(e->get_operands()[1])->get_int_value() - 1;
	if(ub < upper)
	  {
	    upper = ub;
	    u_cause = e;
      ...
      {
	int lb = th->get_theory_data(e->get_operands()[1])->get_int_value();
	if(lb > lower)
	  {
	    lower = lb;
	    l_cause = e;
      ...
      {
	int lb = th->get_theory_data(e->get_operands()[1])->get_int_value() + 1;
	if(lb > lower)
	  {
	    lower = lb;
	    l_cause = e;
  ...
  void export_to_expl(explanation & expl)
  {
	...
	if((*it)->is_disequality())
	  {
	    int v = th->get_theory_data((*it)->get_operands()[1])->get_int_value();
	    if(v > upper || v < lower)
	      continue;
	  }
...
struct multivar_explanation_minimizer {
  ...
  map_type _map;
  csp_theory * _th;
  multivar_explanation_minimizer(csp_theory * th = 0)
    :_map(HASH_TABLE_SIZE),
     _th(th)
  {}

  void reset(csp_theory * th)
  {
