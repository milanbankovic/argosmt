...
bool sum_constraint_handler::calculate_min(int & min)
{
  ...
  for(linear_polynomial::const_iterator it = _lp.begin(), it_end = _lp.end();
    ...
    {
      int coeff = it->second;
      csp_theory::csp_theory_data * c_data = _theory->get_theory_data(it->first);
...
struct min_comparator {
  ...
  {
    int v1_min = _handler->get_theory()->get_theory_data(v1)->get_variable_domain_handler()->get_min();
    int v2_min = _handler->get_theory()->get_theory_data(v2)->get_variable_domain_handler()->get_min();
...
struct max_comparator {
  ...
  bool operator () (const expression & v1, const expression & v2) const
  {
    int v1_max = _handler->get_theory()->get_theory_data(v1)->get_variable_domain_handler()->get_max();
    int v2_max = _handler->get_theory()->get_theory_data(v2)->get_variable_domain_handler()->get_max();
    int v1_coeff = _handler->lp().get_coeff(v1);
    int v2_coeff = _handler->lp().get_coeff(v2);
...
struct abs_coeff_comparator {
  ...
  bool operator () (const expression & v1, const expression & v2) const
  {
    int v1_coeff = _handler->lp().get_coeff(v1);
    int v2_coeff = _handler->lp().get_coeff(v2);
    unsigned v1_index = _handler->get_theory()->get_theory_data(v1)->get_variable_index();
    unsigned v2_index = _handler->get_theory()->get_theory_data(v2)->get_variable_index();
...
struct min_getter {
  csp_theory * _th;
  min_getter(csp_theory * th)
    :_th(th) {}

  int operator () (const expression & var) const
  {
    return _th->get_theory_data(var)->get_variable_domain_handler()->get_min();
  }
...
struct max_getter {
  csp_theory * _th;
  max_getter(csp_theory * th)
    :_th(th) {}

  int operator () (const expression & var) const
  {
    return _th->get_theory_data(var)->get_variable_domain_handler()->get_max();
  }
...
void sum_constraint_handler::init_alldiff_intersections()
{
  ...
  if(_theory->get_alldiff_sum_algorithm() != AS_NONE)
    {
      const expression_vector & constraints = _theory->get_constraints();
      for(unsigned i = 0; i < constraints.size(); i++)
	{
	  ...
	  const expression_set & alldiff_vars = _theory->get_theory_data(constraints[i])->get_constraint_variables();
	  const expression_set & sum_vars = _theory->get_theory_data(_constraint)->get_constraint_variables();
  ...
  if(_theory->get_alldiff_sum_algorithm() == AS_PARTITION)
    {
  ...
  else if(_theory->get_alldiff_sum_algorithm() == AS_BELDICEANU)
    {
  ...
  else if(_theory->get_alldiff_sum_algorithm() == AS_FAST)
    {      
      if(_pos_covered)
	{
...
{
  DECLARE_STATIC(expression_vector, v_temp);
  v_temp.assign(vars.begin(), vars.end());

  while(!v_temp.empty())
    {
      DECLARE_STATIC(expression_vector, clique);
      clique.clear();
	  ...
	  it != it_end; ++it)
	{
	  ...
	  if(_theory->get_solver().get_trail().get_value(alldiff) != EB_TRUE)
	    continue;
	  ...
	  DECLARE_STATIC(expression_vector, curr_clique);
	  curr_clique.clear();
...
void sum_constraint_handler::bel_priority_min(expression_vector & vars, int & val, std::vector<std::pair<expression, int>> & matching)
{
  ...
  DECLARE_STATIC(hash_heap<bel_max_comparator<std::less<int>>>, _heap);
  _heap.set_comparator(bel_max_comparator<std::less<int>>(this));
  _heap.clear();
  unsigned i = 0;
  val = 0;
  int curr_val = _theory->get_theory_data(vars[0])->get_variable_domain_handler()->get_min() - 1;
  for(const expression & var : vars)
    _heap.introduce_element(var);
  
  for(unsigned j = 0; j < vars.size(); j++)
    {
      curr_val = std::max(curr_val + 1, _theory->get_theory_data(vars[j])->get_variable_domain_handler()->get_min());
      for(; i < vars.size() && _theory->get_theory_data(vars[i])->get_variable_domain_handler()->get_min() <= curr_val; i++)
	{
      ...
      val += curr_val;
      if(curr_val > _theory->get_theory_data(top)->get_variable_domain_handler()->get_max())
	{
	  val = 0x7fffffff;
	  return;
	}
...
void sum_constraint_handler::bel_priority_max(expression_vector & vars, int & val, std::vector<std::pair<expression, int>> & matching)
{
  ...
  DECLARE_STATIC(hash_heap<bel_min_comparator<std::greater<int>>>, _heap);
  _heap.set_comparator(bel_min_comparator<std::greater<int>>(this));
  _heap.clear();
  unsigned i = 0;
  val = 0;
  int curr_val = _theory->get_theory_data(vars[0])->get_variable_domain_handler()->get_max() + 1;
  for(const expression & var : vars)
    _heap.introduce_element(var);
  
  for(unsigned j = 0; j < vars.size(); j++)
    {
      curr_val = std::min(curr_val - 1, _theory->get_theory_data(vars[j])->get_variable_domain_handler()->get_max());
      for(; i < vars.size() && _theory->get_theory_data(vars[i])->get_variable_domain_handler()->get_max() >= curr_val; i++)
	{
	  _heap.add_to_heap(vars[i]);
	}
      expression top;
      ...
      matching.push_back(std::make_pair(top, curr_val));
      val += curr_val;
      if(curr_val < _theory->get_theory_data(top)->get_variable_domain_handler()->get_min())
	{
	  val = 0x80000000;
	  return;
	}
					       ...
					       std::vector<std::pair<expression, int>> & matching, std::vector<unsigned> & p)
{
  ...
  DECLARE_STATIC(simple_heap<abs_coeffs_max_comparator>, _heap);                  
  ...
  DECLARE_STATIC(ind_map_type, indices);
  indices.clear();
  ...
  DECLARE_STATIC(expression_vector, next_vars);
  next_vars.clear();
					       ...
					       std::vector<std::pair<expression, int>> & matching, std::vector<unsigned> & p)
{
  ...
  /* Heap of variables (the top variable is with the greatest abs. value of coefficient) */
  DECLARE_STATIC(simple_heap<abs_coeffs_min_comparator>, _heap);
...
bool sum_constraint_handler::calculate_bounds_partition_min(int ubound, int & lp_min, std::unordered_map<expression, int> & cb)
{
  ...
  _theory->_sum_common_data._as_algorithm_time_spent.start();
  ...
  _theory->_sum_common_data._as_algorithm_time_spent.acumulate();
...
bool sum_constraint_handler::calculate_bounds_beldiceanu_min(int ubound, int & lp_min, std::unordered_map<expression, int> & cb)
{
  ...
  DECLARE_STATIC(matching_vector_type, matching);
  matching.clear();
...
bool sum_constraint_handler::calculate_bounds_fast_min(int ubound, int & lp_min, std::unordered_map<expression, int> & cb)
{
  ...
  DECLARE_STATIC(matching_vector_type, matching);
  matching.clear();
  DECLARE_STATIC(std::vector<unsigned>, p);
  p.clear();
  ...
  _theory->_sum_common_data._as_algorithm_time_spent.start();
  DECLARE_STATIC(std::vector<int>, c);
...
bool sum_constraint_handler::calculate_bounds_multifast_max(int lbound, int & lp_max, std::unordered_map<expression, int> & cb)
{
  ...
  DECLARE_STATIC(std::vector<int>, pos_maxs);
  ...
  DECLARE_STATIC(std::vector<int>, neg_maxs);
	...
	if(m_alt > m)
	  {
		...
		it != it_end; ++it)
	      {
		csp_theory::csp_theory_data * c_data = h->_theory->get_theory_data(it->first);
		domain_handler * c_handler = c_data->get_variable_domain_handler();
...
bool sum_constraint_handler::check_upper(int ubound, unsigned trail_size)
{
  ...
  if(ubound < lp_min)
    {      
      ...
      _theory->get_solver().apply_conflict(expl, _theory);
...
void sum_constraint_handler::check_split(int bound, unsigned trail_size)
{
  ...
  if(found == _lp.end())
  ...
  else
    {
      ...
      csp_theory::csp_theory_data * c_data = _theory->get_theory_data(found->first);
      domain_handler * c_handler = c_data->get_variable_domain_handler();
...
{
  const extended_boolean l_value = _theory->get_solver().get_trail().get_value(l);
  ...
  if(l_value == EB_UNDEFINED)
    {
      _theory->get_theory_data(l)->set_explanation_handler(this);      
      _theory->get_solver().apply_propagate(l, _theory);
  ...
  else if(l_value == EB_FALSE)
    {
      ...
      expression l_opp = _theory->get_solver().get_literal_data(l)->get_opposite();
      generate_propagation_explanation(l, conflicting);
      conflicting.push_back(l_opp);
      _theory->get_solver().apply_conflict(conflicting, _theory);
      _common_data->_count_conflict++;
    } 
}
...
{
  extended_boolean eb_value = _theory->get_solver().get_trail().get_value(_constraint);
  if(eb_value == EB_TRUE)
...
{
  generate_explanation(_theory->get_solver().get_trail().size(), bt, expl, expression());
}

sum_constraint_handler::sum_constraint_handler(csp_theory * th, const expression & l_pos, const expression & l_neg)
   ...
   _calculate_bounds_max(nullptr)
{
  ...
  if(fs == function_symbol::LE)
    {
      ...
      _bt_pos = B_UPPER;
      _bt_neg = B_LOWER;
      _bound_pos = _theory->get_value_from_expr(right);
      _bound_neg = _bound_pos + 1;
      _theory->get_polynomial_from_expression(left, _lp);
    }
    ...
    {
      expression eq = _theory->canonize_relation(l_pos);
  ...
  csp_theory::csp_theory_data * cons_data = _theory->get_theory_data(_constraint);
  _all_coeffs_one = true;
  for(linear_polynomial::const_iterator it = _lp.begin(), it_end = _lp.end();
      it != it_end; ++it)
    {
      csp_theory::csp_theory_data * var_data = _theory->add_variable(it->first);
...
{
  if(_theory->get_solver().is_conflict())
    {
      return;
    }

  init_alldiff_intersections();
  extended_boolean eb = _theory->get_solver().get_trail().get_value(_constraint);
  if(eb == EB_UNDEFINED)
  ...
  unsigned trail_size = _theory->get_solver().get_trail().size();
...
{
  assert(_theory->has_theory_data(l));
  explanation expl;
  if(_theory->get_theory_data(l)->has_constraint_data())
    {
	...
	{	  
	  ...
	  expl.push_back(_theory->get_solver().get_trail().get_value(_constraint) == EB_TRUE ?
  ...
  _theory->get_solver().apply_explain(l, expl);
}
...
void sum_constraint_handler::generate_self_explanation(explanation & expl)
{
  for(linear_polynomial::const_iterator it = _lp.begin(), it_end = _lp.end();
      it != it_end; ++it)
    {
      ...
      int coeff = it->second;
      csp_theory::csp_theory_data * v_data = _theory->get_theory_data(var);
      assert(v_data != 0);
      domain_handler * d_handler = v_data->get_variable_domain_handler();
      if(_self_bt == B_BOTH)
	d_handler->explain_domain_at_trail_size(_theory->get_solver().get_trail().get_trail_index(_constraint), expl);
      else if(_self_bt == B_UPPER)
	{
	  if(coeff > 0)
	    d_handler->explain_maximum_at_trail_size(_theory->get_solver().get_trail().get_trail_index(_constraint), expl);
	  else
	    d_handler->explain_minimum_at_trail_size(_theory->get_solver().get_trail().get_trail_index(_constraint), expl);
	}
...
{
  csp_theory::csp_theory_data * c_data = _theory->get_theory_data(c);
...
{
  assert(_theory->get_solver().get_trail().get_value(_constraint) == EB_UNDEFINED);
  ...
  if(holds(min_value, max_value, _bt_pos, _bound_pos))
    {
      ...
      _theory->get_theory_data(_constraint)->set_explanation_handler(this);      
      _theory->get_solver().apply_propagate(_constraint, _theory);
  ...
  else if(holds(min_value, max_value, _bt_neg, _bound_neg))
    {
      ...
      _theory->get_theory_data(_not_constraint)->set_explanation_handler(this);      
      _theory->get_solver().apply_propagate(_not_constraint, _theory);
