...
csp_theory::csp_theory_data::csp_theory_data(csp_expression_type t, unsigned index,
					     csp_theory_data * data)
{
  if(t == CSP_VARIABLE)
    {
...
csp_theory::csp_theory_data::~csp_theory_data()
{
  if(_constraint_data)
    {
...
void csp_theory::on_pop::operator () (const expression & e) const
{
  csp_theory_data * e_data = _theory->get_theory_data(e);
...
csp_theory::csp_theory_data * csp_theory::add_variable(const expression & var)
{
  assert(var->is_function());
  csp_theory_data * var_data = get_theory_data(var);
  if(var_data == 0)
    {
      //std::cout << "VAR: " << var << std::endl;
      var_data = new csp_theory_data(CSP_VARIABLE, _variables.size());
      set_theory_data(var, var_data);
...
csp_theory::csp_theory_data * csp_theory::add_value(const expression & val)
{
  csp_theory_data * val_data = get_theory_data(val);
  if(val_data == 0)
    {
      val_data = new csp_theory_data(CSP_VALUE, _values.size());
      set_theory_data(val, val_data);
...
csp_theory::csp_theory_data * csp_theory::add_value(int v)
{
  expression val;
...
csp_theory::csp_theory_data * csp_theory::add_constraint_pair(const expression & l_pos, const expression & l_neg)
{
  csp_theory_data * l_pos_data = get_theory_data(l_pos);
  if(l_pos_data == 0)
    {
      l_pos_data = new csp_theory_data(CSP_CONSTRAINT, _constraints.size());	  
      csp_theory_data * l_neg_data = new csp_theory_data(CSP_CONSTRAINT, _constraints.size(), l_pos_data);
      set_theory_data(l_pos, l_pos_data);
      set_theory_data(l_neg, l_neg_data);
      _constraints.push_back(l_pos);
    }
  return l_pos_data;
}

csp_theory::csp_theory_data * csp_theory::add_domain_restriction_pair(const expression & l_pos, const expression & l_neg)
{
  csp_theory_data * l_pos_data = get_theory_data(l_pos);
  if(l_pos_data == 0)
    {
      l_pos_data = new csp_theory_data(CSP_DOMAIN_RESTRICTION, _domain_restrictions.size());
      csp_theory_data * l_neg_data = new csp_theory_data(CSP_DOMAIN_RESTRICTION, _domain_restrictions.size(), l_pos_data);
      set_theory_data(l_pos, l_pos_data);
      set_theory_data(l_neg, l_neg_data);
  ...
  for(unsigned i = 0; i < _constraints.size() - 1; i++)
    {
      ...
      csp_theory_data * curr_data = get_theory_data(curr);
...
void csp_theory::add_literal(const expression & l_pos, const expression & l_neg)
{
  if(l_pos->is_equality() ||
      ...
      if(is_atom(left) && is_coefficient(right))
	{
	  ...
	  csp_theory_data * var_data = add_variable(left);
	  csp_theory_data * val_data = add_value(right);
	  var_data->get_variable_domain_handler()->add_value(val_data->get_int_value());
	  csp_theory_data * l_pos_data = add_domain_restriction_pair(l_pos, l_neg);
      ...
      else
	{
	  ...
	  _solver.get_polarity_selection_strategy()->preffered_polarity_hint(l_pos, 1, false);
	  _solver.get_literal_data(l_pos)->add_observing_theory(this);
	  csp_theory_data * l_pos_data = add_constraint_pair(l_pos, l_neg);
...
void csp_theory::new_level() 
{
  _new_level_time_spent.start();
  _trail.new_level();
  _current_level++;
  for(unsigned i = 0; i < _constraints.size(); i++)
    get_theory_data(_constraints[i])->get_constraint_handler()->new_level();
  for(unsigned i = 0; i < _variables.size(); i++)
    get_theory_data(_variables[i])->get_variable_domain_handler()->new_level();
  _varorder->new_level();
  _valorder->new_level();
  clear_all_queues();
...
void csp_theory::backjump(unsigned level) 
{
  _backjump_time_spent.start();
  _trail.backjump(level, on_pop(this));
  _current_level = level;
  for(unsigned i = 0; i < _constraints.size(); i++)
    get_theory_data(_constraints[i])->get_constraint_handler()->backjump(level);
  for(unsigned i = 0; i < _variables.size(); i++)
    get_theory_data(_variables[i])->get_variable_domain_handler()->backjump(level);
...
void csp_theory::add_to_all_queues(constraint_handler * handler)
{
...
void csp_theory::add_to_domain_handler_queue(domain_handler * handler)
{
...
void csp_theory::add_to_inactive_queue(constraint_handler * handler)
{
...
void csp_theory::clear_all_queues()
{
  for(unsigned i = 0; i < _handler_queues.size(); i++)
    clear_queue(i);
  clear_domain_queue();
...
void csp_theory::clear_domain_queue()
{
...
void csp_theory::clear_inactive_queue()
{
...
void csp_theory::clear_queue(unsigned i)
{
...
void csp_theory::assert_literal(const expression & l) 
{
  csp_theory_data * l_data = get_theory_data(l);
  assert(l_data != 0);
  if(l_data->has_domain_restriction_data())
    {
	  ...
	  it != it_end; ++it)
	{
	  csp_theory_data * cons_data = get_theory_data(*it);
...
void csp_theory::init_predefined_variable_bounds()
{
  for(unsigned i = 0; i < _variables.size(); i++)
    {
      csp_theory_data * v_data = get_theory_data(_variables[i]);
...
void csp_theory::check_and_propagate(unsigned layer) 
{
  ...
  if(_solver.all_literals_assigned() && layer == get_num_of_layers() - 1 && _varorder->get_next_variable(var))
    {
      ...
      domain_handler * d_handler = get_theory_data(var)->get_variable_domain_handler();
...
void csp_theory::explain_literal(const expression & l) 
{
  _explain_time_spent.start();
  get_theory_data(l)->get_explanation_handler()->explain_literal(l);
  _explain_time_spent.acumulate();
}


expression csp_theory::canonize_expression(const expression & e)
{
...
void csp_theory::get_literal_pair(const expression & l, expression & l_pos, expression & l_neg)
{
...
void csp_theory::get_model(const expression_vector & exps)
{
  for(unsigned i = 0; i < exps.size(); i++)
    {
      if(!has_theory_data(exps[i]) || !get_theory_data(exps[i])->has_variable_data())
	continue;
      csp_theory_data * var_data = get_theory_data(exps[i]);
      exps[i]->set_assigned(var_data->get_variable_domain_handler()->get_assigned_value());
    }
}

void csp_theory::print_report(std::ostream & ostr)
{
